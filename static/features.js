(() => {
  ...
  const versionLabel = document.querySelector(".sidebar-status small"); if (versionLabel) versionLabel.textContent = "CleanLLM v1.0.14";
  ...
  const tokenMaskObserver=new MutationObserver(()=>{$("#token-list")?.querySelectorAll(".token-value").forEach((el)=>{if(el.dataset.masked)return;const raw=el.textContent||"",masked=raw.length>11?`${raw.slice(0,7)}${"*".repeat(raw.length-11)}${raw.slice(-4)}`:raw;el.dataset.raw=raw;el.dataset.masked="1";el.textContent=masked;const eye=document.createElement("button");eye.className="icon-button";eye.textContent="◉";eye.title="显示/隐藏令牌";eye.onclick=()=>{const visible=el.dataset.visible==="1";el.textContent=visible?masked:el.dataset.raw;el.dataset.visible=visible?"0":"1"};el.parentElement.insertBefore(eye,el.nextSibling);});});$("#token-list")&&tokenMaskObserver.observe($("#token-list"),{childList:true,subtree:true});
