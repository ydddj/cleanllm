(() => {
  ...
  if (typeof pages === "object" && pages.security) pages.security[1] = "系统设置";
  document.querySelector('[data-page="security"] span:last-child')?.replaceChildren("系统设置");
  ...
  const versionLabel = document.querySelector(".sidebar-status small"); if (versionLabel) versionLabel.textContent = "CleanLLM v1.0.43";
  ...
  if (logPage) logPage.insertAdjacentHTML("beforeend", '<button id="clear-logs" class="button" style="color:var(--primary)">清除日志</button>');
  ...
  loadTokens();
  async function loadUsage(){const box=$("#token-usage");try{const data=await request("/api/usage");if($("#dashboard-usage-count")) $("#dashboard-usage-count").textContent=data.today; if(!box)return;$("#usage-total").textContent=`${data.total} 次`;box.innerHTML=`<div class="stats-grid" style="grid-template-columns:repeat(2,1fr)"><article class="stat-card"><span>今日调用</span><strong>${data.today}</strong></article><article class="stat-card"><span>活跃令牌</span><strong>${Object.keys(data.by_token||{}).length}</strong></article></div><h3 style="font-size:14px">使用日志</h3>${(data.logs||[]).length?`<div class="table-wrap"><table class="data-table"><thead><tr><th>时间</th><th>令牌</th><th>接口</th></tr></thead><tbody>${data.logs.map(i=>`<tr><td>${escape(new Date(Number(i.at)*1000).toLocaleString())}</td><td>${escape(i.token_name)}</td><td><code>${escape(i.method)} ${escape(i.path)}</code></td></tr>`).join("")}</tbody></table></div>`:'<p class="form-note">暂无使用记录</p>'}`}catch(e){if(box)box.textContent=e.message; if($("#dashboard-usage-count")) $("#dashboard-usage-count").textContent=""}}
  loadUsage(); setInterval(loadUsage, 10000);
  ...
  if (settingsPanel && !document.querySelector('#appearance-settings')) {
    settingsPanel.insertAdjacentHTML('beforeend','<section id="appearance-settings" class="wide panel appearance-settings"><header class="panel-header"><div><h2>外观</h2><p>首页背景与界面质感</p></div></header><div class="panel-body"><h3>背景图</h3><p class="form-note">上传并保留多张背景图，点击缩略图即可切换。</p><label class="button file-button">上传背景图<input id="appearance-background" type="file" accept="image/*" hidden></label><button id="clear-appearance" class="button" type="button">使用默认背景</button><div id="appearance-gallery" class="appearance-gallery"></div><hr><h3>界面质感</h3><label class="appearance-slider">侧栏透明度 <output id="glass-opacity-value">49%</output><input id="glass-opacity" type="range" min="0" max="100" value="49"></label><label class="appearance-slider">侧栏明暗 <output id="glass-brightness-value">32%</output><input id="glass-brightness" type="range" min="0" max="100" value="32"></label><label class="appearance-slider">蒙版模糊度 <output id="glass-blur-value">13</output><input id="glass-blur" type="range" min="0" max="100" value="13"></label><label class="appearance-slider">蒙版暗度 <output id="mask-opacity-value">69%</output><input id="mask-opacity" type="range" min="0" max="100" value="69"></label><div class="form-footer"><span class="form-note">设置已保存，跨设备一致</span><button id="appearance-reset" class="button" type="button">恢复默认</button><button id="appearance-save" class="button primary" type="button">保存</button></div></div></section>');
    request('/api/settings').then(data=>{ if(data.appearance_background){ document.body.style.backgroundImage=`linear-gradient(rgba(11,13,18,.58),rgba(11,13,18,.7)),url("${data.appearance_background}")`; } const images=data.appearance_backgrounds||[]; const gallery=document.querySelector('#appearance-gallery'); if(gallery) gallery.innerHTML=images.map((img,i)=>`<button class="appearance-thumb ${img===data.appearance_background?'active':''}" data-bg-index="${i}" style="background-image:url('${img}')"></button>`).join(''); ['opacity','brightness','blur','mask-opacity'].forEach(key=>{const el=document.querySelector('#glass-'+key);const val=data['appearance_glass_'+key.replace('-','_')];if(el&&val!==undefined){el.value=val;document.querySelector('#glass-'+key+'-value').textContent=key.includes('opacity')?val+'%':val}}); });
    document.querySelector('#appearance-background').addEventListener('change', async event=>{ const file=event.target.files?.[0]; if(!file)return; if(file.size>5*1024*1024){notify('背景图不能超过 5 MB',true);return} const reader=new FileReader(); reader.onload=async()=>{ try{const current=await request('/api/settings'); current.appearance_background=reader.result; current.appearance_backgrounds=Array.from(new Set([...(current.appearance_backgrounds||[]),reader.result])).slice(-20); await request('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(current)}); document.body.style.backgroundImage=`linear-gradient(rgba(11,13,18,.58),rgba(11,13,18,.7)),url("${reader.result}")`; notify('背景图已保存');}catch(error){notify(error.message,true)} }; reader.readAsDataURL(file); });
    ...
    const sliders=['opacity','brightness','blur','mask-opacity']; sliders.forEach(key=>document.querySelector('#glass-'+key)?.addEventListener('input',event=>{document.querySelector('#glass-'+key+'-value').textContent=event.target.value+(key.includes('opacity')?'%':'')}));
    document.querySelector('#appearance-save')?.addEventListener('click',async()=>{try{const current=await request('/api/settings');sliders.forEach(key=>current['appearance_glass_'+key.replace('-','_')]=Number(document.querySelector('#glass-'+key).value));await request('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(current)});notify('界面质感已保存')}catch(error){notify(error.message,true)}});
    document.querySelector('#appearance-reset')?.addEventListener('click',()=>{const vals={opacity:49,brightness:32,blur:13,'mask-opacity':69};sliders.forEach(key=>{document.querySelector('#glass-'+key).value=vals[key];document.querySelector('#glass-'+key+'-value').textContent=vals[key]+(key.includes('opacity')?'%':'')})});
    document.querySelector('#appearance-gallery')?.addEventListener('click',async event=>{const button=event.target.closest('[data-bg-index]');if(!button)return;const current=await request('/api/settings');current.appearance_background=(current.appearance_backgrounds||[])[Number(button.dataset.bgIndex)];await request('/api/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(current)});document.body.style.backgroundImage=`linear-gradient(rgba(11,13,18,.58),rgba(11,13,18,.7)),url("${current.appearance_background}")`});
  }document.querySelector("#copy-client-endpoint")?.addEventListener("click",async()=>{const value=document.querySelector("#client-endpoint")?.textContent||"";try{await navigator.clipboard.writeText(value)}catch(_){const area=document.createElement("textarea");area.value=value;area.style.position="fixed";area.style.opacity="0";document.body.append(area);area.select();document.execCommand("copy");area.remove()}notify("接口地址已复制")});setTimeout(()=>{document.body.style.visibility="visible"},0);
})();
