...
顶部搜索框会筛选当前页面的卡片、表格与列表，也会匹配表单当前值和占位提示；动态刷新的模型、任务、追踪和日志仍会保持搜索条件。可使用 `Ctrl + K`（macOS 为 `⌘ + K`）快速聚焦搜索框。
...
右上角管理员菜单提供实例“备忘录”，适合临时记录维护安排、待测试模型或上游说明。备忘内容保存在 `/data/settings.json`，同一实例跨设备可见；操作审计只记录备忘录被更新，不记录正文。
...
根目录 `VERSION` 是唯一发布版本来源。推送到 `main` 或手动运行 workflow 后，会自动读取该文件并发布 `用户名/cleanllm:latest` 和当前版本号标签（当前为 `1.3.31`），不再发布 `sha-*` 标签；推送 `v1.0.0` 形式的 Git 标签还会发布对应版本号。镜像同时支持 `linux/amd64` 和 `linux/arm64`。
...
虚拟模型可将客户端稳定别名映射到真实模型，既可指定备用顺序，也可按优先级、权重、令牌名称通配、请求路径通配和本地时间段设置高级路由。上游连续失败达到阈值后会自动熔断，冷却结束后仅允许一个半开放探测请求，成功后恢复；默认阈值为 3 次、冷却 60 秒，可通过诊断中心或 `CIRCUIT_BREAKER_FAILURES`、`CIRCUIT_BREAKER_COOLDOWN_SECONDS` 调整。某个模型返回“无可用渠道”或“模型不存在”只会使该次请求尝试备用路径，不会把同一提供商的其他正常模型一并熔断；真实连接异常、HTTP 429、普通 5xx 和未正常结束的流仍会计入熔断。API令牌支持 RPM、TPM、自然日及自然月 Token 限额，`0` 表示不限制。诊断中心可按请求 ID、模型、令牌和上游筛选追踪，查看模型映射、尝试次数、首字耗时、总耗时及终止原因；兼容性测试覆盖模型列表、Chat Completions、Responses、Embeddings 和两种流式终止事件，且不会保存测试正文或响应。

“分析与成本”按自然日、自然周或自然月汇总请求量、Token、错误率、延迟分位数和估算费用，可按上游、模型或 API令牌分组并导出 CSV。模型价格以 USD / 1M Token 填写，支持模型通配规则、指定上游以及输入、输出和缓存 Token 分别计价。费用在请求发生时写入 SQLite，之后修改价格不会重算历史。API令牌还可设置月度费用预算，并选择达到预算时仅告警或自动停用。
