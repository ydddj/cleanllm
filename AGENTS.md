...
- 流式文本清洗不得对单个增量执行 `strip()`；必须使用每请求独立的跨 SSE Chunk 清洗状态，标签拆包时不得泄漏半截内容；持续过滤期间按需发送 SSE 保活。Responses 流只有收到真实终止事件后才能写入路由记忆，不得将异常 EOF 合成为 `response.completed`。
...
- 单次请求的估算费用写入 `api_usage.cost_usd` 并按请求发生时价格固化；修改模型价格不得重算历史费用。
- 配置快照保存到 SQLite，自动和手动快照合计只保留最近 30 个；恢复前必须先尝试保存当前配置快照。
- Webhook 告警不得包含 API Key、API令牌、密码、请求正文或响应正文，同类告警应避免短时间重复发送。
- Prometheus `/metrics` 只能暴露聚合指标和上游名称，不得暴露令牌名、密钥或敏感配置。
- 停用的上游仍需在管理 UI 中显示，但不得参与模型发现、连通性检测或代理路由；任何保存或批量操作必须保证至少一个上游启用。
- 上游配置导出默认移除所有 API Key；导入数据必须经过与设置保存相同的后端校验。
- API令牌来源白名单支持 IPv4、IPv6 和 CIDR，留空允许全部来源，鉴权时按直连客户端地址校验。
- 操作审计保存到 SQLite，只记录管理员、来源 IP、操作类别、资源、结果和时间，不得保存请求正文、响应内容或敏感配置值。
- 浏览器 SSE 状态连接必须显示连接、重连和断开状态，并使用最高 30 秒的指数退避，恢复联网后立即重连。
...
- 全站命令按钮保持统一：保存、创建、确认、登录、拉取使用主题色实心主按钮；刷新、导入、导出、取消、加载更多使用描边次按钮；删除、清除、撤销、恢复默认等危险操作也使用与普通描边按钮一致的配色，不使用红色或其他强调色，仅保留确认提示和操作语义。普通按钮统一高度、圆角、字号、图标间距及交互状态，图标按钮保持等宽方形。
